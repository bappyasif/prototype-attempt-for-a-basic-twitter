...
function CurrentTrends({handleContentCreators, handleExplicitTrendSearchText}) {
    ...
    // let renderCategories = () => newsCategories.map(name => <TopNewsRelaysUI key={name} newsCategory={name} headlinesCount={handleHeadlinesCount} />)

    // let renderCategories = () => {
    //     return (
    //         showLess
    //         ?
    //         newsCategories.map(name => (headlineCount < 6) && <TopNewsRelaysUI key={name} newsCategory={name} headlinesCount={handleHeadlinesCount} />)
    //         :
    //         newsCategories.map(name => <TopNewsRelaysUI key={name} newsCategory={name} headlinesCount={handleHeadlinesCount} />)
    //     )
    // }
    ...
    // this works, now trying with list of secions from nytimes
    // let randomlySelectNewsCategories = () => {
    //     let randomlySelectedNewsCategories = [];
    //     let rndIdxKey = Math.floor(Math.random() * newsCategories.length)

    //     for(let i=0; i<newsCategories.length; i++) {
    //         randomlySelectedNewsCategories.push(newsCategories[i+rndIdxKey >= newsCategories.length ? (i+rndIdxKey) - newsCategories.length : i+rndIdxKey])
    //     }

    //     randomlySelectedNewsCategories.length == newsCategories.length && setRandomNewsCategories(randomlySelectedNewsCategories)
    //     console.log(randomlySelectedNewsCategories, 'randomly selected news categories!!', rndIdxKey)
    // }
...
// let newsCategories = ['world', 'all', 'science', 'business', 'arts', 'styles', 'home', 'travel', 'others', 'ai', 'geography', 'astrophysics', 'biology', 'physics']
// let newsCategories = ['world', 'all', 'science', 'business', 'arts', 'styles', 'travel', 'geography', 'biology', 'physics']
