...
function SearchSemantics({ searchText }) {
    ...
    let handleDataset = items => {
        let firstHalf = items.slice(0, 6)
        let otherHalf = items.slice(7, 15)
        let rest = items.slice(16)
        setRest(rest)
    ...
    return (
        item
            ...
            <div id='company-information-wrapper'>
                ...
                <div className='company-description'>{companyData && randomIdx != -1 && companyData[randomIdx || 0].short_description}</div>
                <img className='profile-picture' src={unsplashData && randomIdx != -1 && unsplashData[randomIdx || 0].urls.regular} />
            </div>
            :
            null
    )
