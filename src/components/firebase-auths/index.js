import { getAuth, GoogleAuthProvider, sendPasswordResetEmail, signInWithPopup, signInWithPhoneNumber, RecaptchaVerifier, signInWithEmailAndPassword, setPersistence, browserLocalPersistence, browserSessionPersistence, createUserWithEmailAndPassword, signOut, deleteUser, inMemoryPersistence } from 'firebase/auth'
...
// signout and delete user
export let signOutCurrentUser = () => {
    console.log('here!!01')
    
    clearoutUserAuthPersistence()
...
export let deleteCurrentUserAccount = () => {
    let currentUser = auth.currentUser;

    clearoutUserAuthPersistence()

    deleteUser(currentUser)
        .then(() => 'your account is deleted')
        .catch(err => console.log(err.code, err.message))
}

let clearoutUserAuthPersistence = () => {
    setPersistence(auth, inMemoryPersistence)
        .then(() => console.log('persistence is removed now'))
        .catch(err => console.log(err.code, err.message))
}
