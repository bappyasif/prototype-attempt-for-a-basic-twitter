...
function CreateLists() {
    return (
        <div id='create-lists-container'>
            <ListModalHeader icon={leftArrowSvg()} action={'Next'} modalTitle={'Create a new List'} />
            <ListCoverPhoto />
            <ListInformations />
        </div>
    )
...
let ListInformations = () => {
    return (
        <div id='list-information-wrapper'>
            <TextareaComponentWithCount maxLength={25} rows={1} name={'Name'} />
            <TextareaComponentWithCount maxLength={100} rows={2} name={'Description'} />
            <MakeListPrivate />
        </div>
    )
}
...
export let TextareaComponentWithCount = ({ maxLength, rows, name }) => {
    ...
    return (
        <div className='textarea-wrapper' style={{borderColor: focused && 'rgb(29, 155, 240)'}}>
            ...
            <TextareaComponent maxLength={maxLength} rows={rows} handleLength={handleTextAreaLength} handleFocused={handleTextAreaFocused} name={name} focused={focused} />
        </div>
    )
}
...
let TextareaComponent = ({ name, maxLength, rows, handleLength, handleFocused, focused }) => {
    let [text, setText] = useState('')

    let handleChange = evt => setText(evt.target.value)
    
    useEffect(() => handleLength(text.length), [text])
...
let leftArrowSvg = () => <svg className='profile-page-svg-icons'><g><path d="M20 11H7.414l4.293-4.293c.39-.39.39-1.023 0-1.414s-1.023-.39-1.414 0l-6 6c-.39.39-.39 1.023 0 1.414l6 6c.195.195.45.293.707.293s.512-.098.707-.293c.39-.39.39-1.023 0-1.414L7.414 13H20c.553 0 1-.447 1-1s-.447-1-1-1z"></path></g></svg>
